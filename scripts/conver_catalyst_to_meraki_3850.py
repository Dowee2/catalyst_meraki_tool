...
def valid_interface(switch_number, port_number, meraki_serials, intf_name):
            """
...
def map_interface_configs(interfaces, meraki_serials, access_group_number = 0):
    ...
    for intf_name, catalyst_port_config in interfaces.items():
        ...
        try:
            switch_number, port_number = map(int, intf_number.split('/'))
        except ValueError:
            print(f"Invalid interface numbering in {intf_name}")
            continue
        
        if not valid_interface(switch_number, port_number, meraki_serials, intf_name):
            continue
...
if __name__ == "__main__":
    # Example usage
    catalyst_ip = "10.90.240.72"
    meraki_api_key = os.getenv("MERAKI_API_KEY")
    meraki_serials = ["Q5JK-APCF-F9JX"]
