"""
View for comparing MAC address tables between Catalyst and Meraki switches.
"""
...
class MacCompareView:
    """
    View for comparing MAC address tables between switches.
    """
    
    def __init__(self, parent_frame):
        """
        Initialize the MAC comparison view.
    ...
    def _create_results_section(self, parent_frame):
        ...
        columns = ("mac", "catalyst_port", "catalyst_vlan", "meraki_port", "meraki_vlan", "status")
        self.results_tree = ttk.Treeview(tree_frame, columns=columns, show="headings")
        self.results_tree.heading("mac", text="MAC Address")
        self.results_tree.heading("catalyst_port", text="Catalyst Port")
        self.results_tree.heading("catalyst_vlan", text="Catalyst VLAN")
        self.results_tree.heading("meraki_port", text="Meraki Port")
        self.results_tree.heading("meraki_vlan", text="Meraki VLAN")
        self.results_tree.heading("status", text="Status")
        
        self.results_tree.column("mac", width=130)
        self.results_tree.column("catalyst_port", width=100)
        self.results_tree.column("catalyst_vlan", width=90)
        self.results_tree.column("meraki_port", width=90)
        self.results_tree.column("meraki_vlan", width=90)
    ...
    def _apply_filter(self):
        ...
        # Check if we have results to filter
        ...
        for result in self.all_results:
            ...
            if filter_value == "match" and status != "Match":
                continue
            if filter_value == "mismatch" and status != "Port Mismatch":
                continue
            if filter_value == "not_found" and status != "Not Found in Meraki":
                continue
                
            # Add to treeview with appropriate tag
            tag = 'match' if status == 'Match' else 'mismatch' if status == 'Port Mismatch' else 'not_found'
            self.results_tree.insert("", tk.END, values=(
                result.get('MAC_Address', ''),
                result.get('Catalyst_Port', ''),
                result.get('Catalyst_VLAN', ''),
                result.get('Meraki_PortId', ''),
                result.get('Meraki_VLAN', ''),
                status
            ), tags=(tag,))
    ...
    def add_result(self, mac, catalyst_port, catalyst_vlan, meraki_port, meraki_vlan, status):
        """
        Add a result to the treeview.
        
        Args:
            mac (str): The MAC address
            catalyst_port (str): The Catalyst port
            catalyst_vlan (str): The Catalyst VLAN
            meraki_port (str): The Meraki port
            meraki_vlan (str): The Meraki VLAN
            status (str): The status (Match, Port Mismatch, etc.)
        """
        self.results_tree.insert("", tk.END, values=(
            mac,
            catalyst_port,
    ...
    def set_results(self, results):
        ...
        mismatch_count = sum(1 for r in results if r.get('Status', '') == 'Port Mismatch')
        not_found_count = sum(1 for r in results if r.get('Status', '') == 'Not Found in Meraki')
        
        # Update summary
        self.summary_label.config(
            text=f"Results: {len(results)} MAC addresses, {match_count} matches, {mismatch_count} port mismatches, {not_found_count} not found"
        )
